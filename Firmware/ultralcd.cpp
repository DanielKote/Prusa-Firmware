...
static void mFilamentItem_ENCLOSURE()
{
    bFilamentPreheatState = false;
    mFilamentItem(ENCLOSURE_PREHEAT_HOTEND_TEMP, ENCLOSURE_PREHEAT_HPB_TEMP);
}
...
void lcd_generic_preheat_menu()
{
    ...
    else
    {
        ...
        MENU_ITEM_SUBMENU_P(bPreheatOnlyNozzle ? PSTR("ENCL -  " STRINGIFY(ENCLOSURE_PREHEAT_HOTEND_TEMP)): PSTR("ENCL -  " STRINGIFY(ENCLOSURE_PREHEAT_HOTEND_TEMP) "/" STRINGIFY(ENCLOSURE_PREHEAT_HPB_TEMP)), mFilamentItem_ENCLOSURE);
...
//  		if (abs(measured_axis_length[0] - measured_axis_length[1]) > 1) { //check if difference between first and second measurement is low
        if (abs(measured_axis_length[0] - measured_axis_length[1]) > 5) { //check if difference between first and second measurement is low
			//loose pulleys
			const char *_error_1;
